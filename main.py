...
capeta_ubicacion_imagenes = "C:/Users/raul/Desktop/8.- Tesis/7.- Listo Entrenamiento/Muestras/Muestras/COCO/emisores"
...
for i in numeros:
	...
	if isinstance(img,np.ndarray):
		...
		mascara = fn.ruido_gaussiano(mascara,p_noise_shape)
		#Mostramos la máscara desde la cual obtendremos los emisores y sus caracterisiticas
		...
		#Mostramos por pantalla los contornos y las cajas que hemos dibujado
		cv2.imshow("Reconocimiento",reconocimiento)
		...
		for i in cajas_redimensionadas:
			...
			while(True):
				...
				if not(recorte.shape[0] == 0 or recorte.shape[1] == 0):
					...
					#Mostramos el recorte en 200x200
					cv2.imshow("Recorte",resized)
				#Ahora aplicatemos la funcion que caracterizara a el ROI de todas las maneras estudiadas
				# ========================
				caracteristicas = fn.caracterizador(caracterizado,contornos[a-1],1)
				cv2.imshow("Caracterizado",caracterizado)
				# ========================
				...
				categoria,estado,final_ventana = fn.ventana_categoria()
				...
				if estado == True:
					...
					Reflejo = 0
					Emisor_Natural = 0
					if categoria == 1: Emisor = 1
					elif categoria == 2: Reflejo = 1
					elif categoria == 3: Emisor_Natural = 1
					else: break
					diccionario_caracteristicas['Emisor'] = Emisor
					diccionario_caracteristicas['Reflejo'] = Reflejo
					diccionario_caracteristicas['Emisor_Natural'] = Emisor_Natural
